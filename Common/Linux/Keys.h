...
namespace Common {
    ...
    // NAVIGATION KEYS (secuencias de escape ANSI: ESC [ <letra>)
    const std::vector<int> KEY_ARROW_TOP = {27, 91, 65};    // ESC [ A
    const std::vector<int> KEY_ARROW_LEFT = {27, 91, 68};   // ESC [ D
    const std::vector<int> KEY_ARROW_BOTTOM = {27, 91, 66}; // ESC [ B
    const std::vector<int> KEY_ARROW_RIGHT = {27, 91, 67};  // ESC [ C

    // Variante "application mode" (ESC O <letra>), usada por algunas
    // terminales (p. ej. si se activa DECCKM/smkx), incluida WezTerm en
    // ciertos modos.
    const std::vector<int> KEY_ARROW_TOP_2 = {27, 79, 65};    // ESC O A
    const std::vector<int> KEY_ARROW_LEFT_2 = {27, 79, 68};   // ESC O D
    const std::vector<int> KEY_ARROW_BOTTOM_2 = {27, 79, 66}; // ESC O B
    const std::vector<int> KEY_ARROW_RIGHT_2 = {27, 79, 67};  // ESC O C
