...
namespace Tictactoe
{
    ...
    inline char tablero[TAM][TAM];

const int TOTAL_PREGUNTAS = 30;

// Preguntas y opciones del quiz de fútbol
inline std::string preguntas[TOTAL_PREGUNTAS] = {
...
inline std::string opcionA[TOTAL_PREGUNTAS] = {
...
inline std::string opcionB[TOTAL_PREGUNTAS] = {
...
inline std::string opcionC[TOTAL_PREGUNTAS] = {
...
inline std::string opcionD[TOTAL_PREGUNTAS] = {
...
inline char respuestaCorrecta[TOTAL_PREGUNTAS] = {
'B','B','B','B','C','D','B','B','B','B',
'B','C','D','A','A','A','B','A','C','B',
'B','B','B','B','C','B','C','A','C','C'
};

inline bool usada[TOTAL_PREGUNTAS];
inline int preguntasUsadas = 0;

// === Inicialización del tablero ===

/// @brief Agrega el marco interno del tablero (bordes CP437 en filas/columnas 4,8)
inline void agregar_marco(){
    for(int i = 0; i < 13; i++){
        tablero[i][4] = 186;
        tablero[i][8] = 186;
        tablero[4][i] = 205;
...
inline void llenar_tablero_con_espacios(){
    for(int i = 0; i < 13; i++){
        for(int j = 0; j < 13; j++)
            tablero[i][j] = 32;
    }
...
inline void agregarFicha(char pos, char ficha){
...
inline bool celdaVacia(char tecla){
...
inline bool turnoX(int& cursorPos){
    std::vector<int> key;

    // Dibujar una vez al inicio del turno (full redraw on first call)
    DrawBoard(tablero, cursorPos, true);
    ...
    do {
        ...
        if (Common::IsNavigationKey(key)) {
            ...
            // Redibujar solo lo que cambió (snapshot-diff incremental)
            DrawBoard(tablero, cursorPos, false);
            DrawHUD('X', cursorPos, false);
            std::cout.flush();
            ...
            // Cancel — clear screen and redraw everything
...
inline bool turnoO(int& cursorPos){
    std::vector<int> key;
...
inline bool preguntaFutbol(bool& userQuit);

/// @brief Turno del jugador X en modo fútbol
inline bool turnoFutbolX(int& cursorPos){
    // Dibujar tablero y panel quiz al inicio (full redraw)
    DrawBoard(tablero, cursorPos, true);
    ...
    bool userQuit = false;
    if(!preguntaFutbol(userQuit)){
        if (userQuit) {
            if (ExitConfirmView()) return false;
            Common::DrawBackground();
        }
        return true;
    }
...
inline bool turnoFutbolO(int& cursorPos){
    // Dibujar tablero y panel quiz al inicio (full redraw)
    DrawBoard(tablero, cursorPos, true);
...
inline void limpiarPanelQuiz(){
    ClearQuizPanel();
    DrawQuizPanel();
}

/// @brief Presenta una pregunta de fútbol y valida la respuesta
/// @param userQuit Set to true if user pressed Q or ESC to exit
...
inline bool preguntaFutbol(bool& userQuit){
    ...
    // Leer respuesta: A/B/C/D para responder, Q/ESC para salir
    char respuesta = 0;
    userQuit = false;
    while (true) {
        std::vector<int> respKey = Common::ReadConsoleChar();
        if (respKey.empty()) continue;
        if (respKey == Common::KEY_Q || respKey == Common::KEY_Q_LOWER || respKey == Common::KEY_ESCAPE) {
            userQuit = true;
            limpiarPanelQuiz();
            return false;
        }
        respuesta = static_cast<char>(respKey[0]);
...
inline bool jugarNuevamente(){
    DrawPlayAgainPrompt();
    std::vector<int> key = Common::ReadConsoleChar();
    char opcion = static_cast<char>((!key.empty()) ? key[0] : 0);
